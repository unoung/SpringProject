...
import com.sunboard.ch4.domain.BoardDto;
import com.sunboard.ch4.domain.SearchCondition;
import org.junit.*;
...
public class BoardDaoImplTest {
    ...
    public void searchSelectPageTest() throws Exception{
        boardDao.deleteAll();
        for (int i = 1; i<= 20; i++){
            BoardDto boardDto = new BoardDto("title"+i,"abced","abcd"+i);
            boardDao.insert(boardDto);
        }
        SearchCondition sc = new SearchCondition(1,10,"title2","T"); //title20%
        List<BoardDto> list = boardDao.searchSelectPage(sc);
        System.out.println("list = " + list);
        assertTrue(list.size()==2);//1~10, title2, title20

        sc = new SearchCondition(1,10,"title2","W"); //title20%
    ...
    public void searchResultCntTest() throws Exception{
        boardDao.deleteAll();
        for (int i = 1; i<= 20; i++){
            BoardDto boardDto = new BoardDto("title"+i,"abced","abcd");
            boardDao.insert(boardDto);
        }
        SearchCondition sc = new SearchCondition(1,10,"title2","T"); //title20%
        int cnt = boardDao.searchResultCnt(sc);
        System.out.println("cnt = " + cnt);
        assertTrue(cnt == 2); //1~10, title2, title20
    ...
    public void insertTestData() throws Exception{
        boardDao.deleteAll();
        for(int i =1;i<=220;i++){
            BoardDto boardDto = new BoardDto("title" + i, "no content", "abcd");
            boardDao.insert(boardDto);
        }
    }
